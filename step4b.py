...
# Define functions that might be needed (implementations not provided in the MATLAB code)
def get_wind_coord1(ccell, cell_margin):
    # Dummy implementation
    y, x = np.nonzero(ccell)
    y_min, y_max = max(y.min() - cell_margin, 0), min(y.max() + cell_margin, ccell.shape[0])
    x_min, x_max = max(x.min() - cell_margin, 0), min(x.max() + cell_margin, ccell.shape[1])
...
for exp_folder in exp_fold_name:
    mat_path = os.path.join(path_h0, f"{exp_folder}_ART_Track0_122.mat")
    mat_data = scipy.io.loadmat(mat_path)
    
    Mask2 = mat_data['Mask7']  # Assuming Mask7 is in the .mat file

    x_size, y_size = Mask2.shape[0], Mask2.shape[1]
    Ipath = os.path.join(im_path, exp_folder)
    ...
    scipy.io.savemat(os.path.join(save_fold, name3), {
        'ALLDATA': ALLDATA,
        'all_back': all_back,
