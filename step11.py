...
tet_file_list = sorted(file_list)
tet = load_mat(os.path.join(path, tet_file_list[0]))

file_list = [f for f in os.listdir(tet_track_path) if '_TET_ID' in f]
tet_id_file_list = sorted(file_list)
tet_ids = load_mat(os.path.join(path, tet_id_file_list[0]))  # Load TET IDs

# Load ART Tracks
art_track_path = path
file_list = [f for f in os.listdir(art_track_path) if '_ART_Track_DS' in f]
art_file_list = sorted(file_list)
...
for i in range(len(masks)):
    if masks[i].size > 0:
        masks[i] = resize(masks[i].astype(np.float64), ART[i].shape, order=0, preserve_range=True).astype(np.uint16)
    ...
    for ccell in range(TET_obj):
        if tet_ids['TET_ID'][ccell, 0] != -1:
            if dead_tets[ccell] == 0:
                if TET_exists[1, ccell] >= shock_period[1, 0] + 1:
                    mask = masks[int(shock_period[1, 0]) + 1] == ccell
                else:
                    mask = masks[int(TET_exists[ccell, 1])] == ccell
        ...
        for its in int1:
            ...
            if I11.sum() > 0:
                ...
                if 1 <= kx <= 2:
                    I11 *= 1000
            I12 += I11
        I13 = (I12 > 0).astype(np.uint16) * I3.astype(np.uint16).T
    ...
    for iv in range(int(tet['TET_obj'][0, 0])):
        if tet_ids['TET_ID'][0, iv] != -1:
            if dead_tets[iv] == 0:
                ...
                T2 = (I3.astype(np.uint16) * T1.astype(np.uint16).T)
